...
/* Stream frames in */
   void TrbBridge::_importFrame(const TrbBridgeFrame & frame, unsigned int retry) {
      if (!retry) return;
   ...
   void TrbBridge::importTimeslice(const fles::Timeslice& ts) {
      for (size_t c = 0; c < ts.num_components(); ++c) {
         ...
         for (size_t m = 0; m < ts.num_microslices(c); ++m) {
            ...
            while(msInteralPtr < msDescriptor.size / sizeof(*msContent)) {
               ...
               _importFrame( TrbBridgeFrame(&msContent[msInteralPtr], word_count*2) );
      ...
      if (_lastStatDump != now) {
         std::cout << _currentStats.dump();
         _lastStatDump = now;
      }

// i dont care about duplication of mc in timeslices, so just discard a transaction between two timeslices !
      _transaction.reset();
   }
